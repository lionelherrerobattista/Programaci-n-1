        ...
        if(order==0 || order==1)
        {
            ...
            for(i=1;i<ll_len(this);i++)
            {
                ...
                if(order==1)
                {
                    ...
                    while(j>=0 && pFunc(auxElementI,ll_get(this,j))<0)//si la funcion devuelve  menor a 0 ordeno
                    {
                ...
                {
                    //Comparo con el de la izquierda
                    while(j>=0 && pFunc(auxElementI,ll_get(this,j))>0)//si la funcion devuelve mayor a 0 ordeno
                    {
                        auxElementJ=ll_get(this, j);
                        ll_set(this, j+1, auxElementJ);
                        //muevo todo para la derecha
